  ...
  return (
    <div className='cnx-root'>
      ...
      <section id='contact' className='cnx-section'>
        ...
        <p className='cnx-note' style={{ marginTop: 12 }}>
          We dont chase -- we build. From vision to results.♾️
        </p>
      </section>
      ...
      {showWallets && (
        <div
          className='cnx-modal-overlay'
          onClick={() => setShowWallets(false)}
        >
          <div className='cnx-modal' onClick={(e) => e.stopPropagation()}>
            {wallets.map((w) => (
              <div className='cnx-wallet' key={w.name}>
                {w.icon}
                <div className='cnx-wallet-name'>{w.name}</div>
                <div className='cnx-wallet-addr'>{w.address}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <style>{`
        ...
        *{box-sizing:border-box} html,body,#root{height:100%}
        ...
        .cnx-btn{display:inline-block; padding:10px 14px; border-radius:14px; text-decoration:none; transition:.2s; border:1px solid rgba(245,199,107,.45); color:#fff; cursor:pointer; background:transparent}
        ...
        /* Wallet Modal */
        .cnx-modal-overlay{position:fixed; inset:0; background:rgba(0,0,0,.7); display:flex; justify-content:center; align-items:center; z-index:50}
        .cnx-modal{background:rgba(255,255,255,.06); padding:24px; border-radius:14px; border:1px solid rgba(255,255,255,.2); width:90%; max-width:400px}
        .cnx-wallet{display:flex; flex-direction:column; align-items:center; margin-bottom:16px}
        .cnx-wallet:last-child{margin-bottom:0}
        .cnx-wallet-name{margin-top:4px; font-weight:600}
        .cnx-wallet-addr{font-size:12px; color:#cfd3dc; word-break:break-all; text-align:center; margin-top:2px}
