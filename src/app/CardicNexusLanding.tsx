    ...
    try {
      const uploadData = await (async () => {
        ...
        if (
          ...
          !uploadJson?.url ||
          !uploadJson.key ||
          typeof uploadJson.size !== 'number' ||
          typeof uploadJson.originalName !== 'string' ||
          typeof uploadJson.mimeType !== 'string'
        ) {
          const message =
            uploadJson?.error || 'We could not upload your proof.';
          throw new Error(message);
        }

        return uploadJson as {
          url: string;
          key: string;
          originalName: string;
          mimeType: string;
      ...
      const response = await fetch('/api/register', {
        ...
        body: JSON.stringify({
          ...
          proofOriginalName: uploadData.originalName,
          proofMimeType: uploadData.mimeType,
          proofFileSize: uploadData.size,
        }),
      });
