  ...
  try {
    ...
    if (!name || !email || !country)
      return NextResponse.json(
        { ok: false, error: 'Missing name, email, or country' },
        { status: 400 }
      );
    ...
    upsertParticipant({
      ...
      status: 'PENDING',
      createdAt: now,
      meta: { platform, broker, server, leverage, accountSize, country },
    });
    ...
    const userHtml = `
      <div style="background:#0a0f1c;padding:24px;font-family:Inter,Arial;color:#cbd5e1">
        <div style="max-width:560px;margin:auto;background:#0f172a;border:1px solid #1e293b;border-radius:14px;padding:24px">
          ...
          <p style="margin-top:16px">Status: <b>PENDING</b>. Your submission is locked in — our desk will prepare your MT4/MT5 account and link FXBlue. This may take up to 24 hours.</p>
    ...
    const adminHtml = `
      <div>
        ...
        <p><b>Name:</b> ${name}<br/><b>Email:</b> ${email}<br/><b>Country:</b> ${country}<br/><b>Token:</b> ${token}</p>
